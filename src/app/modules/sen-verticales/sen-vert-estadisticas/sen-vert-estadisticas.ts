...
@Component({
    selector: 'app-sen-vert-estadisticas',
    standalone: true,
    imports: [CommonModule, FormsModule, RouterModule],
    templateUrl: './sen-vert-estadisticas.html',
    styleUrl: './sen-vert-estadisticas.scss',
    encapsulation: ViewEncapsulation.None
})
export class SenVertEstadisticasComponent implements OnInit, OnDestroy {
    loading = true;
    error = '';
    ...
    load() {
        ...
        this.senVertService.getEstadisticas(q).subscribe({
            next: (res) => {
    ...
    canvasId(secId: string, bloqueId: string, orient: 'h' | 'v' | 'd'): string {
        return `sv-${secId}-${bloqueId}-${orient}`.replace(/[^a-zA-Z0-9_-]/g, '_');
    }
    ...
    descargarGrafico(secId: string, bloqueId: string, orient: 'h' | 'v' | 'd') {
        ...
        a.href = url;
        a.download = `sen-vert_${bloqueId}_${orient}.png`;
        a.click();
    }
    ...
    exportarExcel() {
        ...
        const metaRows: (string | number)[][] = [
            ['INFRAVIAL — Estadísticas Señales verticales (ExistSenVert)'],
        ...
        XLSX.writeFile(wb, `estadisticas-sen-vert_${new Date().toISOString().slice(0, 10)}.xlsx`);
    }
                ...
                datasets: [
                    {
                        ...
                        data: values,
                        backgroundColor: bg,
                        borderColor: t.light ? 'rgba(124,58,237,0.28)' : 'rgba(15,20,40,0.5)',
                        borderWidth: 1,
                        borderRadius: 6
                    }
                ]
